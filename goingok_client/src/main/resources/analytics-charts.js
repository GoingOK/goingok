...
const HtmlContainers = /** @class */ (function () {
...
export function buildAnalyticsCharts(entries) {
    const htmlContainer = new HtmlContainers();
    ...
    entries.forEach(function (c) {
        const uniqueUsers = Array.from(d3.rollup(c.value, function (d) {
            return d.length;
        }, function (d) {
            return d.pseudonym;
        }), function (_a) {
    ...
    const groupChart = setGroupChart(data);
    preRender(groupChart);
    renderGroupChart(groupChart, data);
    function setGroupChart(data) {
        const result = new Chart();
        result.id = "groups-chart";
        const containerDimensions = chartFunctions.getContainerDimension(result.id);
    ...
    function renderGroupChart(chart, data) {
        ...
        function onMouseover(e, d) {
            ...
            const tooltipBox = chartFunctions.tooltip.appendTooltipBox(chart);
            ...
            function xTooltip(x, tooltipBox) {
                const xTooltip = chart.x.scale(x) + chart.x.scale.bandwidth();
            ...
            function yTooltip(y, tooltipBox) {
                const yTooltip = chart.y.scale(y) - (tooltipBox.node().getBBox().height / 2);
        ...
        function onClick(e, d) {
            ...
            const groupsStatisticsCard = chartFunctions.appendCard(htmlContainer.groupStatistics, "Statitics (" + d.group + ")");
            ...
            const timelineCard = chartFunctions.appendCard(htmlContainer.groupTimeline, "Reflections vs Time (" + d.group + ")");
            ...
            const compareCard = chartFunctions.appendCard(htmlContainer.compare, "Compare " + d.group + " with:");
        ...
        //Enable sort
        const sortButton = chart.renderElements.svg.select(".y-label-container").attr("class", "y-label-container zoom");
        const yArrow = chartFunctions.sort.appendArrow(sortButton, chart, false, true);
    ...
    function groupTimeline(data) {
        const timelineChart = setTimelineChart(data);
        ...
        d3.select("#group-timeline #timeline-plot").on("click", function (e) {
            ...
            if (selectedOption == "density") {
                const timelineChart_1 = setTimelineChart(data);
                renderTimelineDensity(timelineChart_1, data);
            }
            if (selectedOption == "scatter") {
                const timelineChart_2 = setTimelineChart(data);
                const timelineZoomChart = setTimelineZoomChart(timelineChart_2, data);
        ...
        function renderTimelineDensity(chart, data) {
            ...
            const densityData = d3.contourDensity()
                .x(function (d) {
                    return chart.x.scale(d.timestamp);
                })
                .y(function (d) {
                    return chart.y.scale(d.point);
                })
                .bandwidth(5)
                .thresholds(20)(data);
            //Draw contours
            ...
            function zoomed(e) {
                const newChartRange = [0, chart.width - chart.padding.yAxis].map(function (d) {
                    return e.transform.applyX(d);
                });
        ...
        function renderTimelineScatter(chart, zoomChart, data) {
            ...
            function onMouseover(e, d) {
                ...
                function xTooltip(x, tooltipBox) {
                    const xTooltip = chart.x.scale(x);
                    if (chart.width - chart.padding.yAxis < xTooltip + tooltipBox.node().getBBox().width) {
                ...
                ;
                function yTooltip(y, tooltipBox) {
                    const yTooltip = chart.y.scale(y) - tooltipBox.node().getBBox().height - 10;
                    if (yTooltip < 0) {
    ...
    function groupViolinChart(data, group) {
        ...
        const groupData = d3.filter(data, function (d) {
            return currentGroups.includes(d.group);
        });
        const currentData = [];
        groupData.forEach(function (c) {
            const userAvg = Array.from(d3.rollup(c.value, function (d) {
                return Math.round(d3.mean(d.map(function (r) {
                    return r.point;
                })));
            }, function (d) {
                return d.pseudonym;
            }), function (_a) {
                const pseudonym = _a[0], point = _a[1];
        ...
        const violinChart = setViolinChart(groupData, "group-violin-chart");
        preRender(violinChart);
        renderViolin(violinChart, groupData, 30, 70);
        const violinUsersChart = setViolinChart(currentData, "group-violin-users-chart");
        ...
        function renderViolin(chart, data, tDistressed, tSoaring) {
            ...
            const thresholdAxis = chartFunctions.axis.setThresholdAxis(chart, tDistressed, tSoaring);
            //Draw threshold axis
            ...
            function draggingSoaring(e, d) {
                ...
                chart.renderElements.contentContainer.selectAll(".threshold-axis")
                    .call(thresholdAxis);
                const positionX = chart.width - chart.padding.yAxis - chart.padding.right + 5;
                let positionY = chart.y.scale(tSoaring) + 25;
                const indicator = chart.renderElements.contentContainer.select(".threshold-indicator-container.soaring");
            ...
            function dragEndSoaring(e, d) {
                ...
                const newBin = d3.bin().domain([0, 100]).thresholds([0, tDistressed, newT]);
            ...
            function draggingDistressed(e, d) {
                ...
                chart.renderElements.contentContainer.selectAll(".threshold-axis")
                    .call(thresholdAxis);
                const soaringIndicator = chart.renderElements.contentContainer.select(".threshold-indicator-container.soaring");
                ...
                const indicator = chart.renderElements.contentContainer.select(".threshold-indicator-container.distressed")
            ...
            function dragEndDistressed(e, d) {
                ...
                const newBin = d3.bin().domain([0, 100]).thresholds([0, newT, tSoaring]);
                chart.renderElements.content.remove();
                chart.renderElements.content = binContainer.append("path")
                    .attr("id", chart.id + "-violin")
                    .attr("class", "violin-path")
            ...
            //Create bins             
            const bin = d3.bin().domain([0, 100]).thresholds([0, tDistressed, tSoaring]);
        ...
        d3.selectAll("#group-compare input").on("change", function (e) {
            if (e.target.checked) {
                currentGroups.push(e.target.id);
            }
            else {
                currentGroups.splice(currentGroups.indexOf(e.target.id), 1);
            }
            const groupData = d3.filter(data, function (d) {
                return currentGroups.includes(d.group);
            });
    ...
    function groupCompare(data, group) {
        ...
        const currentGroups = [group];
        const groupsData = d3.filter(data, function (d) {
            return !currentGroups.includes(d.group);
        });
        const container = d3.select("#group-compare .card-body");
        groupsData.forEach(function (c) {
            container.html(container.html() + ("<div class=\"form-check\">\n                                <input class=\"form-check-input\" type=\"checkbox\" value=\"" + c.group + "\" id=\"" + c.group + "\" />\n                                <label class=\"form-check-label\" for=\"" + c.group + "\">" + c.group + "</label>\n                            </div>"));
        });
    }
...
var chartFunctions = {
    ...
    appendContentContainer: function (chart) {
        const result = chart.renderElements.svg.append("g")
    ...
    axis: {
        ...
        appendThresholdLabel: function (chart) {
            const label = chart.renderElements.svg.append("g")
        ...
        appendThresholdIndicators: function (chart, thresholds) {
            thresholds.forEach(function (c, i) {
                const indicator = chart.renderElements.contentContainer.append("g")
                ...
                const box = indicator.append("rect")
                    .attr("class", "threshold-indicator-box " + (i == 0 ? "distressed" : "soaring"));
                const text = indicator.append("text")
    ...
    tooltip: {
        ...
        appendTooltipText: function (chart, title, values) {
            const result = chart.renderElements.contentContainer.select(".tooltip-container").append("text")
                .attr("class", "tooltip-text title")
                .attr("x", 10)
                .text(title);
            const textSize = result.node().getBBox().height;
    ...
    sort: {
        ...
        arrowPoints: function (svg, chart, x, y) {
            const selector = x == true ? ".x-label-text" : ".y-label-text";
            const height = svg.select(selector).node().getBBox().height;
            const width = svg.select(selector).node().getBBox().width;
    ...
    click: {
        ...
        appendText: function (chart, d, title, values) {
            const container = chart.renderElements.contentContainer.append("g")
                .attr("class", "click-container");
            const box = container.append("rect")
                .attr("class", "click-box");
            const text = container.append("text")
                .attr("class", "click-text title")
                .attr("x", 10)
                .text(title);
            const textSize = text.node().getBBox().height;
    ...
    drag: {
        ...
        appendThresholdPercentages: function (chart, binContainer, bin, bandwithScale, tDistressed, tSoaring) {
            const binData = function (data) {
                const bins = bin(data.map(function (r) {
                    return r.point;
                }));
                const result = [];
            ...
            const binTextContainer = binContainer.append("g")
                .attr("class", chart.id + "-violin-text-container");
            const binTextBox = binTextContainer.selectAll("rect")
                .data(function (d) {
                    return binData(d.value);
                })
                .enter()
                .append("rect")
                .attr("class", "violin-text-box");
            const binText = binTextContainer.selectAll("text")
                .data(function (d) {
                    return binData(d.value);
                })
                .enter()
